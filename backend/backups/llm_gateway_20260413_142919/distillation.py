...
{{"questions": ["pregunta 1", "pregunta 2", ...]}}"""
    ...
    async def _query_single_master(
        ...
        if not isinstance(question, str) or not question.strip():
            print(f"[DISTILL] Pregunta inválida: {question}")
            return {}
        
        max_retries = 3
        for attempt in range(max_retries):
            try:
                async with httpx.AsyncClient(timeout=float(DISTILL_MASTER_TIMEOUT_SECONDS)) as client:
                    r = await client.post(
                    ...
                    if r.status_code == 200:
                        ...
                        print(f"[DISTILL] HTTP {r.status_code} para {model} (intento {attempt+1})")
