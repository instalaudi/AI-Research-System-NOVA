...
class AgentService:
    ...
    async def start_research_task(self, interest_areas: List[str], user_id: int, background_tasks: Any):
        ...
        for area in interest_areas:
            # 1. Start research — user_forced permite la cadena completa aunque swarm_research esté off en modo autónomo
            await task_queue.add_task(
                "start_research",
                {"interest_areas": [area], "user_forced": True},
                topic=area,
                user_id=user_id,
            )
            # 2. Master Model Distillation (Background) — solo si el panel permite destilación
